...
def derivative_diameter_vls(y, x,
                            PARAMETERS_VLS):
    """ Calculate the derivative of the diameter in Tersoff's model at a given height and length
    :param y: radius
    :param x:
    :param PARAMETERS_VLS: constant set of parameters
    :return: derivative for ODE
    """
    r = y
...
def gr_vs(t, gr_vs_tot, GR_AX, LAMBDA, y):
    """
    Calculate the VS growth rate at a given time and height
    :param t: time in min
    :param gr_vs_tot: total VS growth rate
    :param GR_AX: axial growth rate
    :param LAMBDA: diffusion length
    :param y: height
    :return: VS growth rate
    """
    y_total = GR_AX * t
    if y < y_total:
...
def size_untapered(tapering_series_at_t, diameter_untapered, length_untapered, taper_untapered, ratio_untapered,
                   radius_total_top, TIME_RANGE,
                   R_i):
    """
    Calculate the diameter, length, flux ratio, and top diameter of the untapered NW of a time series
    :param tapering_series_at_t:
    :param diameter_untapered:
    :param length_untapered:
    :param taper_untapered:
    :param ratio_untapered:
    :param radius_total_top:
    :param TIME_RANGE:
    :param R_i:
    :return:
    """
...
for R_i in RATIO_RANGE:
    ...
    radius_vls = integrate.odeint(derivative_diameter_vls, RADIUS_0, X_RANGE_VLS,
                                  args=(PARAMETERS_VLS,))  # solve ODE for VLS model
    ...
    for t_i in TIME_RANGE:
        ...
        taper_i = -1 * (radius_total[len(x) - 1] - radius_total[
            6]) / x_total_stop * 100  # taper at one point, compared to x=500nm
        tapering_series_at_t.append(taper_i)  # calculate taper values of time series

    diameter_untapered, length_untapered, taper_untapered, ratio_untapered = size_untapered(tapering_series_at_t,
                                                                                            diameter_untapered,
                                                                                            length_untapered,
                                                                                            taper_untapered,
                                                                                            ratio_untapered,
...
fname = "Map-time%d_%d_%d-ratio%d_%d_%d-lambda%d.csv" % (
TIME_MIN, TIME_MAX, TIME_INCREMENT, RATIO_MIN, RATIO_MAX, RATIO_INCREMENT, LAMBDA)
fname_size = "size-flux%d_%d_%d-ratio%d_%d_%d-lambda%d.csv" % (
TIME_MIN, TIME_MAX, TIME_INCREMENT, RATIO_MIN, RATIO_MAX, RATIO_INCREMENT, LAMBDA)
...
im = ax1.imshow(tapering_for_ratio_time, extent=[TIME_MIN, TIME_MAX, RATIO_MIN, RATIO_MAX], interpolation='none',
                cmap="bwr", vmin=np.amin(tapering_for_ratio_time), vmax=-np.amin(tapering_for_ratio_time),
                origin=['lower'])
plt.colorbar(im)
ax1.set_aspect('auto')
...
plt.savefig(fname='figure.pdf', format='pdf')
plt.show()
